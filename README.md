...
1. 指定时间点 arrive_time
2. interval, ms。
3. is_repeated 是否重复执行
4. is_cancled  取消标志
5. task


cancle()             取消定时任务
cancleRepeated()     取消重复执行
```
...
#### 2.5 IO 线程   Refactor  进程只负责接受客户端的连接   线程负责处理客户端的请求
创建一个IO 线程，他会帮我们执行：
...
class {
 ...
 pthread_t m_thread;    //进程ID
 pid_t m_thread_id;     //线程ID
 EventLoop event_loop;  //当前线程的event_loop
}
...
void RpcChannel::CallMethod(const google::protobuf::MethodDescriptor* method,
  ...
  if (my_controller->GetMsgId().empty()) {
    ...
    // 这样的目的是为了实现 m_msg_id 的透传，假设服务 A 调用了 B，那么同一个 msgid 可以在服务 A 和 B 之间串起来，方便日志追踪
    std::string m_msg_id = RunTime::GetRunTime()->m_msgid;
    if (!m_msg_id.empty()) {
      req_protocol->m_m_msg_id = m_msg_id;
      my_controller->SetMsgId(m_msg_id);
    } else {
      req_protocol->m_m_msg_id = MsgIDUtil::GenMsgID();
      my_controller->SetMsgId(req_protocol->m_m_msg_id);
    }

  } else {
    // 如果 controller 指定了 msgno, 直接使用
    req_protocol->m_m_msg_id = my_controller->GetMsgId();
  }

  req_protocol->m_method_name = method->full_name();
  INFOLOG("%s | call method name [%s]", req_protocol->m_m_msg_id.c_str(), req_protocol->m_method_name.c_str());

  if (!m_is_init) {
    std::string err_info = "RpcChannel not call init()";
    my_controller->SetError(ERROR_RPC_CHANNEL_INIT, err_info);
    ERRORLOG("%s | %s, RpcChannel not init ", req_protocol->m_m_msg_id.c_str(), err_info.c_str());
    callBack();
    return;
  }
  ...
  if (!request->SerializeToString(&(req_protocol->m_pb_data))) {
    ...
    ERRORLOG("%s | %s, origin requeset [%s] ", req_protocol->m_m_msg_id.c_str(), err_info.c_str(), request->ShortDebugString().c_str());
    callBack();
    return;
  }
  ...
  m_client->connect([req_protocol, this]() mutable {
    ...
    if (getTcpClient()->getConnectErrorCode() != 0) {
      ...
      ERRORLOG("%s | connect error, error coode[%d], error info[%s], peer addr[%s]", 
        req_protocol->m_m_msg_id.c_str(), my_controller->GetErrorCode(), 
    ...
    getTcpClient()->writeMessage(req_protocol, [req_protocol, this, my_controller](AbstractProtocol::s_ptr) mutable {
      INFOLOG("%s | send rpc request success. call method name[%s], peer addr[%s], local addr[%s]", 
        req_protocol->m_m_msg_id.c_str(), req_protocol->m_method_name.c_str(),
        getTcpClient()->getPeerAddr()->toString().c_str(), getTcpClient()->getLocalAddr()->toString().c_str());

        getTcpClient()->readMessage(req_protocol->m_m_msg_id, [this, my_controller](AbstractProtocol::s_ptr msg) mutable {
        ...
        INFOLOG("%s | success get rpc response, call method name[%s], peer addr[%s], local addr[%s]", 
          rsp_protocol->m_m_msg_id.c_str(), rsp_protocol->m_method_name.c_str(),
          getTcpClient()->getPeerAddr()->toString().c_str(), getTcpClient()->getLocalAddr()->toString().c_str());

        if (!(getResponse()->ParseFromString(rsp_protocol->m_pb_data))){
          ERRORLOG("%s | serialize error", rsp_protocol->m_m_msg_id.c_str());
...
void TcpConnection::excute() {
  if (m_connection_type == TcpConnectionByServer) {
    ...
    for (size_t i = 0;  i < result.size(); ++i) {
      ...
      INFOLOG("success get request[%s] from client[%s]", result[i]->m_m_msg_id.c_str(), m_peer_addr->toString().c_str());

      std::shared_ptr<TinyPBProtocol> message = std::make_shared<TinyPBProtocol>();
      // message->m_pb_data = "hello. this is rocket rpc test data";
      // message->m_m_msg_id = result[i]->m_m_msg_id;
...
void RpcDispatcher::dispatch(AbstractProtocol::s_ptr request, AbstractProtocol::s_ptr response, TcpConnection* connection) {
  ...
  rsp_protocol->m_m_msg_id = req_protocol->m_m_msg_id;
  ...
  if (it == m_service_map.end()) {
    ERRORLOG("%s | sericve neame[%s] not found", req_protocol->m_m_msg_id.c_str(), service_name.c_str());
  ...
  if (method == NULL) {
    ERRORLOG("%s | method neame[%s] not found in service[%s]", req_protocol->m_m_msg_id.c_str(), method_name.c_str(), service_name.c_str());
  ...
  if (!req_msg->ParseFromString(req_protocol->m_pb_data)) {
    ERRORLOG("%s | deserilize error", req_protocol->m_m_msg_id.c_str(), method_name.c_str(), service_name.c_str());
    setTinyPBError(rsp_protocol, ERROR_FAILED_DESERIALIZE, "deserilize error");
    DELETE_RESOURCE(req_msg);
    return;
  }

  INFOLOG("%s | get rpc request[%s]", req_protocol->m_m_msg_id.c_str(), req_msg->ShortDebugString().c_str());
  ...
  rpc_controller->SetMsgId(req_protocol->m_m_msg_id);

  RunTime::GetRunTime()->m_msgid = req_protocol->m_m_msg_id;
  ...
  RpcClosure* closure = new RpcClosure(nullptr, [req_msg, rsp_msg, req_protocol, rsp_protocol, connection, rpc_controller, this]() mutable {
    if (!rsp_msg->SerializeToString(&(rsp_protocol->m_pb_data))) {
      ERRORLOG("%s | serilize error, origin message [%s]", req_protocol->m_m_msg_id.c_str(), rsp_msg->ShortDebugString().c_str());
      setTinyPBError(rsp_protocol, ERROR_FAILED_SERIALIZE, "serilize error");
    } else {
      rsp_protocol->m_err_code = 0;
      rsp_protocol->m_err_info = "";
      INFOLOG("%s | dispatch success, requesut[%s], response[%s]", req_protocol->m_m_msg_id.c_str(), req_msg->ShortDebugString().c_str(), rsp_msg->ShortDebugString().c_str());
    }
