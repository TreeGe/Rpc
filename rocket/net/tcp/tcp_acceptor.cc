...
namespace rocket
{
    ...
    {
        if (!local_addr->checkVaild()) // 需要校验传入地址的有效性
        {
        ...
        // 把监听套接字设为非阻塞
        int val = 1;
        if (setsockopt(m_listenfd, SOL_SOCKET, SO_REUSEADDR, &val, sizeof(val)) == -1) // SO_REUSEADDR防止端口被占用的问题
        {
            ERRORLOG("setsockopt REUSEADDR error,error=%d,error=%s", errno, strerror(errno));
        }

        socklen_t len = m_local_addr->getSockLen();
        if (bind(m_listenfd, m_local_addr->getSockAddr(), len) != 0)
        {
            ERRORLOG("bind error error=%d, error=%s", errno, strerror(errno));
            exit(0);
        }

        if (listen(m_listenfd, 1000) != 0)
        {
            ERRORLOG("listen error , error=%d , error =%s", errno, strerror(errno));
            exit(0);
        }
    }
    ...
    std::pair<int,NetAddr::s_ptr> TcpAcceptor::accept()
    {
        if (m_family == AF_INET)
        {
            ...
            if(client_fd<0)
            {
                ERRORLOG("accept error , error=%d , error=%s",errno,strerror(errno));
            }
